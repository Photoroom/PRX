...
class LPIPS(Algorithm):
    ...
    Args:
        ...
        t_threshold: Only apply LPIPS for timesteps below this value.
    """
    ...
    def add_new_pipeline_modules(self, model: torch.nn.Module) -> None:
        ...
        print(f" > LPIPS: Added frozen {self.lpips_net} network in bf16 (weight={self.lpips_weight})")
    ...
    def apply(self, event: Event, state: State, logger: Logger) -> None:
        ...
        print(" > LPIPS: Registered denoiser hook and wrapped loss()")
    ...
    def _wrap_loss_method(self, state: State, model: torch.nn.Module) -> None:
        ...
        def augmented_loss(outputs: Dict[str, torch.Tensor], batch: Dict[str, Any]) -> torch.Tensor:
            ...
            model.logger.log_metrics({"loss/train/lpips": lpips_val.detach().cpu()})
